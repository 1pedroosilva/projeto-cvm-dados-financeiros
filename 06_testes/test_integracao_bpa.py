...
# MAGIC %md
# MAGIC # Teste de Integração E2E - Pipeline BPA (Bronze → Silver)
# MAGIC
# MAGIC **Propósito**: Validar execução ponta a ponta do pipeline BPA  
...
# Critério: COUNT(*) > 0 na tabela Bronze BPA
# ============================================================================

print("✅ Validando: Tabela Bronze existe e tem dados...")

try:
    tabela_bronze = f"{SCHEMA_BRONZE}.102_bpa_dfp"
...
# Critério: COUNT(*) > 0 na tabela Silver BPA
# ============================================================================

print("✅ Validando: Tabela Silver existe e tem dados...")

try:
    tabela_silver = f"{SCHEMA_SILVER}.202_bpa_dfp"
...
try:
    # a) Calcular excedentes na Bronze pela mesma chave da Window Function do 202
...
# Nota: ORDEM_EXERC é parte da PK porque a BPA traz exercício corrente (ÚLTIMO)
#       e anterior (PENÚLTIMO) para a mesma conta e data de referência.
#       A Window Function do 202 particiona por essas 4 colunas.
# ============================================================================
...
if not falhas:
    ...
    print(f"   \u2022 Ambiente: '{AMBIENTE}'")
    print(f"\n\u2713 Pipeline BPA funcionando corretamente (Bronze \u2192 Silver)\n")
else:
    print("="*80)
