...
# MAGIC %md
# MAGIC # Ingestão de Dados Financeiros - Balanço Patrimonial Ativo
# MAGIC
# MAGIC ## Objetivo
# MAGIC Este notebook realiza a **ingestão de dados de Balanço Patrimonial Ativo (BPA) consolidado** de companhias abertas brasileiras, publicados pela Comissão de Valores Mobiliários (CVM) através das Demonstrações Financeiras Padronizadas (DFP).
# MAGIC
...
# MAGIC * **Demonstração específica**: Balanço Patrimonial Ativo (BPA) - ativos consolidados
# MAGIC
# MAGIC ## Conteúdo
...
for ano in ANOS_PROCESSAR:
    ...
    versao_atual = spark.sql(f"""
        ...
        FROM proj_cvm_01_bronze.102_bpa_dfp
...
# DBTITLE 1,CARGA APPEND-ONLY NA BRONZE
    ...
    df_bronze.write \
        .format("delta") \
        .mode("append") \
        .saveAsTable("proj_cvm_01_bronze.102_bpa_dfp")
...
print(f"\n{'='*80}")
print(f"BRONZE BPA - PROCESSAMENTO CONCLUÍDO")
